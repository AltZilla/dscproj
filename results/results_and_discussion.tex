...
\subsubsection{Time-of-Use Pricing} The electricity pricing follows a three-tier TOU structure:
\begin{equation}
    p(h) = \begin{cases}
...
\subsubsection{$\beta$-VAE Loss Function} The training objective maximizes the Evidence Lower Bound (ELBO) with a controlled KL weight $\beta = 0.5$:
\begin{equation}
...
\subsubsection{Federated Averaging (FedAvg)} In each communication round $r$, a random subset $S_r$ of $\lceil 0.4K \rceil = 20$ clients is selected. Each client $k \in S_r$ trains a local copy of the global model for $E = 5$ local epochs using its private data:
\begin{equation}
...
\subsubsection{Evaluation Metrics} Model performance is measured using three complementary metrics:
\begin{equation}
...
\subsubsection{Leader (Grid Operator) Objective} The leader maximizes profit subject to supply capacity $K = 150$ kW:
\begin{equation}
...
\subsubsection{Equilibrium Computation} The Stackelberg equilibrium $p^*$ is found via iterative grid search with progressive refinement. In each of 50 iterations, the price range is narrowed around the current best, evaluating 30 candidate prices per iteration. The equilibrium satisfies:
\begin{equation}
...
\subsubsection{Initial Population} A population of $N_p = 100$ candidate solutions is initialized. The first 20\% are seeded from the user's preferred schedule with 10--30\% random bit flips to ensure diversity:
\begin{equation}
