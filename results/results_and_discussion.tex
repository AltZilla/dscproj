...
\thanks{S.~K.~Linga and M.~VP are with the School of Computer Science
and Engineering, Vellore Institute of Technology, Vellore, India.}% <-this % stops a space
...
\begin{abstract}
This paper presents a privacy-preserving hybrid intelligent framework for energy optimization in IoT-enabled smart homes. The proposed system integrates six sequential stages: a Digital Twin Simulator for synthetic data generation, a $\beta$-Variational Autoencoder ($\beta$-VAE) for latent feature extraction, a Federated Learning-based Gated Recurrent Unit (FedGRU) network for privacy-preserving load forecasting, a Hierarchical Stackelberg Game for dynamic electricity pricing, a Differential Evolution (DE) optimizer for constrained multi-objective schedule optimization, and an interactive visualization dashboard. Experimental results on a synthetic dataset of 50 smart homes over 30 days demonstrate that the framework achieves 46.2\% daily energy cost reduction, 23.0\% peak load reduction, and robust forecasting accuracy (MAE = 1.846\,kW) while preserving user data privacy through federated learning. Comparative evaluation against three recent IEEE baselines confirms balanced superiority across cost savings, comfort preservation, privacy, convergence speed, peak reduction, and forecast accuracy.
\end{abstract}

\begin{IEEEkeywords}
Smart home energy management, federated learning, privacy-preserving load forecasting, Stackelberg game, differential evolution, $\beta$-VAE, IoT, demand response.
\end{IEEEkeywords}
...
\textit{Occupancy Model.} Occupant behavior is modeled as a 3-state discrete-time Markov chain with states $\mathcal{S} = \{s_0 = \text{Home}, s_1 = \text{Away}, s_2 = \text{Sleeping}\}$. The occupancy state at time $t$ evolves according to:
\begin{equation}
    P(X_{t+1} = s_j | X_t = s_i) = \mathbf{M}_\tau(i, j)
    \label{eq:markov}
\end{equation}
where $X_t$ is the occupancy random variable at time step $t$, $s_i$ and $s_j$ are occupancy states from $\mathcal{S}$, and $\mathbf{M}_\tau \in \mathbb{R}^{3 \times 3}$ is the time-dependent transition matrix indexed by $(i, j)$. The time period $\tau \in \{\text{night}, \text{day}, \text{evening}\}$ selects the appropriate matrix based on the current hour. The time-varying matrices capture realistic behavioral patterns: high sleep probability during night hours ($\mathbf{M}_{\text{night}}[0,2] = 0.8$), high away probability during day ($\mathbf{M}_{\text{day}}[0,1] = 0.35$), and high home probability during evening ($\mathbf{M}_{\text{evening}}[1,0] = 0.6$).

\textit{Thermal Environment Model.} The outdoor temperature follows a sinusoidal daily cycle with stochastic perturbation:
\begin{equation}
...
\textit{Appliance Activation Model.} Each appliance $a$ has a time-varying activation probability modulated by occupancy and environmental conditions:
\begin{equation}
...
\textit{Time-of-Use Pricing.} The electricity pricing follows a three-tier TOU structure:
\begin{equation}
    p(h) = \begin{cases}
...
\textit{Outlier Detection.} Outliers are detected and clipped using the Interquartile Range (IQR) method:
\begin{equation}
...
\textit{Cyclical Time Encoding.} Time features are encoded as cyclical sine-cosine pairs to preserve periodicity:
\begin{equation}
...
\textit{MinMax Normalization.} All features are normalized to $[0, 1]$:
\begin{equation}
...
\textit{Encoder Network.} The encoder $q_\phi(z|x)$ maps input $\mathbf{x} \in \mathbb{R}^{19}$ to the parameters of a Gaussian posterior:
\begin{equation}
...
\end{equation}
where $\mathbf{h}$ is the hidden representation, $\mathbf{W}_1$ and $\mathbf{b}_1$ are the weight matrix and bias of the first fully connected layer, $\text{BN}(\cdot)$ denotes batch normalization, $\text{ReLU}(\cdot) = \max(0, \cdot)$ is the rectified linear activation, $\boldsymbol{\mu}$ and $\log \boldsymbol{\sigma}^2$ are the mean and log-variance vectors of the latent Gaussian posterior, and $\mathbf{W}_\mu$, $\mathbf{b}_\mu$, $\mathbf{W}_\sigma$, $\mathbf{b}_\sigma$ are their respective projection weights and biases. The architecture uses two hidden layers with dimensions $64 \to 32 \to 16$.

\textit{Reparameterization Trick.} To enable gradient-based optimization through the stochastic sampling, we use:
\begin{equation}
    \mathbf{z} = \boldsymbol{\mu} + \boldsymbol{\sigma} \odot \boldsymbol{\epsilon}, \quad \boldsymbol{\epsilon} \sim \mathcal{N}(\mathbf{0}, \mathbf{I})
    \label{eq:reparam}
\end{equation}
where $\mathbf{z} \in \mathbb{R}^{d_z}$ is the sampled latent vector, $\boldsymbol{\mu}$ and $\boldsymbol{\sigma}$ are the encoder-predicted mean and standard deviation, $\boldsymbol{\epsilon}$ is a noise vector sampled from the standard normal distribution $\mathcal{N}(\mathbf{0}, \mathbf{I})$, and $\odot$ denotes element-wise (Hadamard) multiplication.

\textit{Decoder Network.} The decoder $p_\theta(x|z)$ reconstructs the input from the latent code: $\hat{\mathbf{x}} = \sigma(\mathbf{W}_3(\text{ReLU}(\mathbf{W}_2 \mathbf{z} + \mathbf{b}_2)) + \mathbf{b}_3)$, where $\sigma(\cdot)$ is the sigmoid activation.

\textit{$\beta$-VAE Loss Function.} The training objective maximizes the Evidence Lower Bound (ELBO) with a controlled KL weight $\beta = 0.5$:
\begin{equation}
...
\textit{GRU Cell Equations.} Each GRU cell at time step $t$ computes:
\begin{equation}
...
\textit{Evaluation Metrics.} Model performance is measured using three complementary metrics:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:mape}
\end{equation}
where $n$ is the total number of test samples, $y_i$ is the $i$-th ground truth power consumption value, $\hat{y}_i$ is the corresponding model prediction, and the MAPE threshold $|y_i| > 0.1$ excludes near-zero ground truth values that would cause division instability.
...
\textit{Follower (User) Utility.} Each user $i$ has a concave utility function with diminishing returns:
\begin{equation}
    U_i(c_i) = w_i \sqrt{c_i}
    \label{eq:user_utility}
\end{equation}
where $U_i(c_i)$ is the utility of user $i$, $c_i$ is the user's electricity consumption in kW, and $w_i \sim 15.0 \cdot \mathcal{U}(0.8, 1.2)$ is a randomized comfort weight sampled from a uniform distribution to model heterogeneous user preferences. Given price $p$ (in INR/kWh), user $i$ maximizes net utility $U_i(c_i) - p \cdot c_i$.

\textit{Follower Best Response.} Setting $\frac{\partial}{\partial c_i}[U_i - p \cdot c_i] = 0$:
\begin{equation}
    \frac{w_i}{2\sqrt{c_i^*}} = p \implies c_i^*(p) = \min\left(\left(\frac{w_i}{2p}\right)^2, \; C_{\max,i}\right)
    \label{eq:best_response}
\end{equation}
where $c_i^*(p)$ is the optimal consumption of user $i$ as a function of price $p$, $w_i$ is the user's comfort weight, and $C_{\max,i}$ is the maximum consumption capacity derived from appliance rated powers scaled by a realistic capacity factor ($\sim$4.9\,kW per home). The $\min$ operator clips the response to prevent physically infeasible consumption.

\textit{Leader (Grid Operator) Objective.} The leader maximizes profit subject to supply capacity $K = 150$ kW:
\begin{equation}
    \max_p \; \Pi(p) = p \cdot \sum_{i=1}^{N} c_i^*(p) - \frac{1}{2}\max\left(0, \sum_{i=1}^{N}c_i^*(p) - K\right)^2
    \label{eq:leader_obj}
\end{equation}
where $\Pi(p)$ is the leader's profit function, $p$ is the price set by the leader, $N = 50$ is the number of follower users, $c_i^*(p)$ is each follower's best response consumption, and $K = 150$\,kW is the total supply capacity. The first term is revenue, and the second is a quadratic penalty for exceeding grid capacity.

\textit{Equilibrium Computation.} The Stackelberg equilibrium $p^*$ is found via iterative grid search with progressive refinement. In each of 50 iterations, the price range is narrowed around the current best, evaluating 30 candidate prices per iteration. The equilibrium satisfies:
\begin{equation}
    \frac{\partial \Pi}{\partial p}\bigg|_{p=p^*} = 0, \quad p^* \in [p_{\min}, p_{\max}] = [3, 15] \; \text{INR/kWh}
    \label{eq:equilibrium}
\end{equation}
where $\Pi$ is the leader's profit, $p^*$ is the Stackelberg equilibrium price, $\frac{\partial \Pi}{\partial p}$ is the marginal profit with respect to price, and $p_{\min} = 3$ and $p_{\max} = 15$\,INR/kWh bound the feasible price range.

\textit{Hourly Stackelberg.} The game runs independently for each of 24 hours, with hourly demand profiles derived from the GRU forecaster:
\begin{equation}
...
\textit{Initial Population.} A population of $N_p = 100$ candidate solutions is initialized. The first 20\% are seeded from the user's preferred schedule with 10--30\% random bit flips to ensure diversity:
\begin{equation}
    ...
    \label{eq:init_pop}
\end{equation}
where $\mathbf{S}_i^{(0)}$ is the $i$-th initial candidate schedule, $\mathbf{S}_{\text{pref}}$ is the user's preferred (default) schedule, $\oplus$ denotes element-wise XOR (bit-flip) operation, $\mathcal{B}(\rho)$ denotes a Bernoulli mask where each element is 1 with probability $\rho$, and $N_p = 100$ is the population size.

\textit{Mutation.} For each individual $\mathbf{S}_i$, a mutant vector is generated using DE/rand/1:
\begin{equation}
    \mathbf{v}_i = \mathbf{S}_{r_1} + F \cdot (\mathbf{S}_{r_2} - \mathbf{S}_{r_3})
    \label{eq:mutation}
\end{equation}
where $\mathbf{v}_i$ is the continuous mutant vector for individual $i$, $\mathbf{S}_{r_1}$, $\mathbf{S}_{r_2}$, $\mathbf{S}_{r_3}$ are three distinct randomly selected individuals from the population, $F = 0.8$ is the mutation scaling factor, and the subtraction $\mathbf{S}_{r_2} - \mathbf{S}_{r_3}$ produces the difference vector. The continuous mutant is binarized via a sigmoid threshold: $\mathbf{v}_i^{\text{bin}} = \mathbb{1}[\sigma(5(\mathbf{v}_i - 0.5)) > 0.5]$, where $\mathbb{1}[\cdot]$ is the indicator function and $\sigma(\cdot)$ is the sigmoid function.

\textit{Binomial Crossover.} A trial vector is formed by component-wise selection:
\begin{equation}
    u_{i,j} = \begin{cases}
...
\textit{Multi-Objective Fitness Function.} The fitness function aggregates eight weighted penalty terms:
\begin{equation}
\begin{aligned}
...
\end{equation}
where $E_{\text{total}} = \sum_{a,h} S_{a,h} \cdot P_a \cdot \Delta t$ is the total daily energy consumption, $S_{a,h}$ is the ON/OFF state of appliance $a$ at slot $h$, $P_a$ is the rated power, $\Delta t = 0.25$\,h is the time slot duration, $E_{\max} = 80$\,kWh is the maximum allowable daily energy, $L_h = \sum_a S_{a,h} \cdot P_a$ is the instantaneous load at hour $h$, and $L_{\max} = 15$\,kW is the maximum allowable hourly power. The feasible set defining all strict operational states forms a rugged discontinuous subspace:
\begin{equation}
...
\end{equation}
where $\Omega$ is the feasible solution space, $\mathbf{S}$ is a binary schedule matrix of $A = 7$ appliances over $T = 96$ time slots, $\Phi_{\text{constraints}}(\mathbf{S})$ is the constraint violation penalty, and $\epsilon_{\text{tol}}$ is a small numerical tolerance threshold for feasibility.
...
Fig.~\ref{fig:cost_reduction} presents the daily energy cost reduction percentages across five methods. The proposed framework achieves $46.2\%$ cost reduction (1308\,INR $\to$ 704\,INR) compared to the unoptimized baseline. The cost reduction is computed as $\Delta C = (C_{\text{orig}} - C_{\text{opt}}) / C_{\text{orig}} \times 100\%$, where $\Delta C$ is the percentage cost reduction, $C_{\text{orig}}$ is the original daily cost, $C_{\text{opt}}$ is the optimized cost, and $C = \sum_{a,h} S_{a,h} \cdot P_a \cdot p_h \cdot \Delta t$ with $S_{a,h}$ being the ON/OFF state, $P_a$ the rated power, $p_h$ the price at hour $h$, and $\Delta t$ the time slot duration. The proposed framework's advantage is threefold: (1) the Stackelberg pricing provides optimal price signals that the DE directly optimizes against, creating a tighter feedback loop than static prices used by BDIA and STFL; (2) the 8-term fitness function captures multi-dimensional trade-offs that simple cost-only objectives miss; and (3) the VAE-GRU forecast alignment penalty nudges schedules toward predicted demand patterns, establishing a stable fixed point theorem bounded by:
\begin{equation}
...
Fig.~\ref{fig:par} presents the Peak-to-Average Ratio (PAR) across six configurations. The proposed DE-based optimizer achieves $23.0\%$ peak load reduction. The PAR metric is defined as $\text{PAR} = L_{\max} / \bar{L}$, where $L_{\max} = \max_h \sum_a S_{a,h} \cdot P_a$ is the maximum hourly load across all time slots, $\bar{L} = \frac{1}{24}\sum_h \sum_a S_{a,h} \cdot P_a$ is the average hourly load, $S_{a,h}$ is the ON/OFF state of appliance $a$ at hour $h$, and $P_a$ is the rated power of appliance $a$. A PAR approaching 1.0 indicates perfectly uniform load distribution. The improvement is driven by the DE fitness function's peak penalty $\Phi_{\text{peak}} = \max(0, \frac{L_{\max}}{\bar{L}} - 1.5) \times 30$ and grid capacity penalty $\Phi_{\text{grid}} = \sum_h \max(0, L_h - K)^2 \times 50$, where $L_h$ is the load at hour $h$ and $K$ is the grid capacity.
...
Fig.~\ref{fig:load_dist} visualizes the hourly load profiles before and after optimization. The proposed framework demonstrates significant peak shaving during expensive hours as the DE's cost term $\sum S_{a,h} P_a p_h$ becomes prohibitively expensive for deferrable loads, where $S_{a,h}$ is the ON/OFF state, $P_a$ is the rated power, and $p_h$ is the price at hour $h$. Valley filling occurs as the optimizer shifts EV charging (7.0\,kW) and water heating (4.5\,kW) to exploit low off-peak prices. The load standard deviation across hours $\sigma_L = \sqrt{\frac{1}{24}\sum_h (L_h - \bar{L})^2}$, where $L_h$ is the load at hour $h$ and $\bar{L}$ is the mean hourly load, is substantially lower for the proposed framework compared to all baselines, confirming superior load balancing.
...
Fig.~\ref{fig:radar} presents a radar chart comparing the four frameworks across six normalized performance dimensions. The aggregate multi-objective score is computed as $\mathcal{S} = \frac{1}{M}\sum_{m=1}^{M} s_m$, where $\mathcal{S}$ is the overall score, $M = 6$ is the number of performance dimensions, and $s_m \in [0,1]$ is the normalized score on the $m$-th dimension. The radar area, computed as $A = \frac{1}{2}\sum_{m=1}^{M} s_m \cdot s_{m+1} \cdot \sin(\frac{2\pi}{M})$, where $A$ is the polygon area and $s_{M+1} = s_1$ wraps cyclically, provides a geometric measure of overall performance. The BDIA approach scores poorly on Privacy due to its centralized learning. MADRL achieves strong Comfort through reward shaping but sacrifices Convergence Speed. STFL achieves good Privacy through federated design but lacks game-theoretic pricing and evolutionary optimization. The proposed framework's balanced superiority validates the complementary integration of all four algorithmic pillars: VAE (representation), FedGRU (privacy-preserving forecasting), Stackelberg (optimal pricing), and DE (constrained schedule optimization).
...
This paper presented a privacy-preserving hybrid intelligent framework for smart home energy optimization that integrates six complementary modules: a Digital Twin Simulator for realistic data generation, a $\beta$-VAE for nonlinear dimensionality reduction, a Federated GRU network for privacy-preserving load forecasting, a Hierarchical Stackelberg Game for dynamic pricing, a Differential Evolution optimizer for constrained multi-objective schedule optimization, and an interactive visualization dashboard.

Experimental evaluation on a synthetic dataset of 50 smart homes over 30 days demonstrated that the proposed framework achieves 46.2\% daily energy cost reduction (1308\,INR $\to$ 704\,INR), 23.0\% peak load reduction, and robust forecasting accuracy (MAE = 1.846\,kW, RMSE = 2.502\,kW) while preserving complete data privacy through federated learning. Comparative analysis against three recent IEEE baselines---BDIA, MADRL, and STFL---confirmed balanced superiority across all six evaluation dimensions: cost savings, comfort preservation, privacy, convergence speed, peak reduction, and forecast accuracy.

The key contribution lies in the end-to-end integration of these algorithmic pillars into a unified pipeline, where VAE-compressed latent representations enable efficient federated training, Stackelberg equilibrium prices provide economically rational cost signals, and the DE optimizer produces schedules that simultaneously minimize cost, preserve comfort, and respect physical constraints. Future work will focus on full-scale hardware deployment, physical IoT sensor integration, edge-node real-time federated deployment, and large-scale real-user validation.
...
\bibitem{bdia2025}
``Big data-intelligence analytics for energy optimization in IoT-enabled smart home devices,'' \textit{IEEE Trans.}, 2025. [Online]. Available: \url{https://ieeexplore.ieee.org/document/10980008}

\bibitem{madrl2025}
``Multiagent DRL-based demand response optimization for IoT-based smart home energy management systems,'' \textit{IEEE Trans.}, 2025. [Online]. Available: \url{https://ieeexplore.ieee.org/document/10967490}

\bibitem{stfl2025}
``Spatiotemporal federated learning for privacy-preserving load forecasting and appliance scheduling in smart city homes,'' \textit{IEEE Trans.}, 2025. [Online]. Available: \url{https://ieeexplore.ieee.org/document/11189875}
\end{thebibliography}
