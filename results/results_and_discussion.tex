...
% ============================================================
\begin{abstract}
The proliferation of Internet-of-Things (IoT) devices enables real-time energy management, but accurate load forecasting and appliance scheduling remain constrained by the conflict between data privacy and forecasting precision. Centralized learning risks exposing sensitive consumer behavioral data, while decentralized methods often sacrifice accuracy and lack cost signals. Existing solutions, such as static pricing and independent reinforcement learning agents, address these challenges in isolation, failing to jointly optimize privacy, forecast accuracy, grid stability, and user comfort under a unified and physically constrained architecture.

This paper presents a six-stage hybrid intelligent framework integrating a Digital Twin Simulator, a $\beta$-Variational Autoencoder ($\beta = 0.5$) for dimensionality reduction, and a Federated Gated Recurrent Unit (FedGRU) network for distributed privacy-preserving load forecasting. This is coupled with a Hierarchical Stackelberg Game for dynamic pricing and a Differential Evolution optimizer with a composite fitness function for constrained multi-objective appliance scheduling. The framework operates end-to-end on raw IoT data, enforcing data locality through the FedAvg protocol across 50 smart home clients to guarantee absolute raw data privacy.

Experimental evaluation on 144,000 records spanning 50 homes over 30 days shows that the proposed framework achieves a 46.2\% daily energy cost reduction and a 23.0\% peak load reduction. It maintains robust forecasting accuracy with a Mean Absolute Error of 1.846\,kW after 20 federated rounds. Comparative analysis against three recent IEEE baselines confirms its balanced superiority in simultaneously optimizing cost efficiency, grid stability, and consumer privacy within an integrated and secure optimization pipeline.
\end{abstract}
...
We introduce a highly constrained multi-objective Differential Evolution (DE/rand/1/bin) optimizer evaluating an eight-term custom fitness function to simultaneously execute exact peak-shaving, thermal comfort preservation, and physical constraint adherence.
